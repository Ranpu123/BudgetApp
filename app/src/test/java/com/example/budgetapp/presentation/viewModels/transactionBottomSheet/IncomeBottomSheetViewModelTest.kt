...
class IncomeBottomSheetViewModelTest: KoinTest{
    private var context: Context = mockk(relaxed = true)
    private lateinit var viewModel: IncomeBottomSheetViewModel
    private val repository: IIncomeRepository = mockk(relaxed = true)
    ...
    @Test
    fun `when checkform() is successful should add new income`() = runTest{
        ...
        coVerify(exactly = 1) { repository.addIncome(any()) }
    }

    @Test
    fun `when checkform() fails should not add new income`() = runTest{
        ...
        coVerify(exactly = 0) { repository.addIncome(any()) }
    }
}