...
class RemoveFixedIncomeWorkerTest{
    ...
    fun when_data_is_not_set_should_fail() {
        val worker = TestListenableWorkerBuilder<RemoveFixedIncomeWorker>(context)
            .build()
        runBlocking {
    ...
    fun should_succeed(){
        ...
        coEvery { restApi.removeFixedIncome(any()) } returns mockResponse
        coEvery { dao.hardRemove(any<RoomFixedIncome>()) } returns 1
    ...
    fun when_IOException_thrown_should_retry(){
        ...
        coEvery { dao.getFixedIncome(any()) } returns RoomFixedIncome.fromFixedIncome(input,1)
        coEvery { restApi.removeFixedIncome(any()) } throws IOException("Error")
    ...
    fun when_HttpException_thrown_should_retry(){
        ...
        coEvery { dao.getFixedIncome(any()) } returns RoomFixedIncome.fromFixedIncome(input,1)
        coEvery { restApi.removeFixedIncome(any()) } throws HttpException(errResponse)
    ...
    fun when_Exception_thrown_should_fail(){
        ...
        coEvery { dao.getFixedIncome(any()) } returns RoomFixedIncome.fromFixedIncome(input,1)
        coEvery { restApi.removeFixedIncome(any()) } throws Exception("Error")
