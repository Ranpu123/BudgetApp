...
class AddIncomeWorkerTest{
    ...
    fun when_data_is_not_set_should_fail() {
        val worker = TestListenableWorkerBuilder<AddIncomeWorker>(context)
            .build()
        runBlocking {
    ...
    fun should_succeed(){
        ...
        coEvery { restApi.addIncome(any()) } returns mockResponse
        coEvery { dao.update(any<RoomIncome>()) } returns 1
    ...
    fun when_IOException_thrown_should_retry(){
        ...
        coEvery { dao.getIncome(any()) } returns RoomIncome.fromIncome(input,1)
        coEvery { restApi.addIncome(any()) } throws IOException("Error")
    ...
    fun when_HttpException_thrown_should_retry(){
        ...
        coEvery { dao.getIncome(any()) } returns RoomIncome.fromIncome(input,1)
        coEvery { restApi.addIncome(any()) } throws HttpException(errResponse)
    ...
    fun when_Exception_thrown_should_fail(){
        ...
        coEvery { dao.getIncome(any()) } returns RoomIncome.fromIncome(input,1)
        coEvery { restApi.addIncome(any()) } throws Exception("Error")
