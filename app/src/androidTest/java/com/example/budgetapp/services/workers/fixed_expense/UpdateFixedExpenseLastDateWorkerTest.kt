...
class UpdateFixedExpenseLastDateWorkerTest{
    ...
    fun when_data_is_not_set_should_fail() {
        val worker = TestListenableWorkerBuilder<UpdateFixedExpenseLastDateWorker>(context)
            .build()
        runBlocking {
    ...
    fun should_succeed(){
        ...
        coEvery { restApi.updateFixedExpenseLastDate(any(), any()) } returns mockResponse
        coEvery { dao.update(any<RoomFixedExpense>()) } returns 1
    ...
    fun when_IOException_thrown_should_retry(){
        ...
        coEvery { dao.getFixedExpense(any()) } returns RoomFixedExpense.fromFixedExpense(input,1)
        coEvery { restApi.updateFixedExpenseLastDate(any(), any()) } throws IOException("Error")
    ...
    fun when_HttpException_thrown_should_retry(){
        ...
        coEvery { dao.getFixedExpense(any()) } returns RoomFixedExpense.fromFixedExpense(input,1)
        coEvery { restApi.updateFixedExpenseLastDate(any(), any()) } throws HttpException(errResponse)
    ...
    fun when_Exception_thrown_should_fail(){
        ...
        coEvery { dao.getFixedExpense(any()) } returns RoomFixedExpense.fromFixedExpense(input,1)
        coEvery { restApi.updateFixedExpenseLastDate(any(), any()) } throws Exception("Error")
    ...
    fun when_404_should_retry(){
        ...
        val input = TransactionsTestHelper.fixedExpenses.first()
        val errResponse: Response<FixedExpense> = mockk()
        every { errResponse.message() } returns "Error"
        every { errResponse.code() } returns 404
        every { errResponse.isSuccessful } returns false
        coEvery { dao.getFixedExpense(any()) } returns RoomFixedExpense.fromFixedExpense(input,1)
        coEvery { restApi.updateFixedExpenseLastDate(any(), any()) } returns errResponse
