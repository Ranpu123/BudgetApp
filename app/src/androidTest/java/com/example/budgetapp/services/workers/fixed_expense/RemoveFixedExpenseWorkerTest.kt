...
class RemoveFixedExpenseWorkerTest{
    ...
    fun when_data_is_not_set_should_fail() {
        val worker = TestListenableWorkerBuilder<RemoveFixedExpenseWorker>(context)
            .build()
        runBlocking {
    ...
    fun should_succeed(){
        ...
        coEvery { restApi.removeFixedExpense(any()) } returns mockResponse
        coEvery { dao.hardRemove(any<RoomFixedExpense>()) } returns 1
    ...
    fun when_IOException_thrown_should_retry(){
        ...
        coEvery { dao.getFixedExpense(any()) } returns RoomFixedExpense.fromFixedExpense(input,1)
        coEvery { restApi.removeFixedExpense(any()) } throws IOException("Error")
    ...
    fun when_HttpException_thrown_should_retry(){
        ...
        coEvery { dao.getFixedExpense(any()) } returns RoomFixedExpense.fromFixedExpense(input,1)
        coEvery { restApi.removeFixedExpense(any()) } throws HttpException(errResponse)
    ...
    fun when_Exception_thrown_should_fail(){
        ...
        coEvery { dao.getFixedExpense(any()) } returns RoomFixedExpense.fromFixedExpense(input,1)
        coEvery { restApi.removeFixedExpense(any()) } throws Exception("Error")
